...
return (/^(19\d{2}|20\d{2}|2100)$/).test(year);
}
...
function checkEmail(email) {
    return (/^[a-zA-Z\.-]+@[a-zA-Z\.]+\.[a-z]{2,3}$/).test(email);
}
...
function checkDomainUrl(domain) {
   return (/^http(s)?:\/\/[\w\._-]+\.[a-z]{2,4}$/).test(domain);
}
...
function createLinksFromDomains(domain) {
  let reg = new RegExp('(http|https):\\/\\/([\\w\\.-]+\\.[a-z]{2,3})', 'g');
  let arr = domain.match(reg);
  arr.forEach(elem => domain = domain.replace(elem, `<a href="${elem}" target="_blank">${elem.replace(/^(http|https):\/\//gi, '')}</a>`));
  return domain;
}
